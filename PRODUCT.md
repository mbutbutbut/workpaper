...
- Confirmed: the bookkeeper reviews (Accept, or Reopen with a required message that returns the item to the admin as Waiting on me); every item has a small conversation, tied to the transaction so it survives re-imports; people can edit or remove their own messages (a removed message leaves a "Message removed" placeholder); the admin can delete transactions in bulk (they return as new rows if a later import lists them again); the one-line note stays as the item's current reason; admins manage people in the app (add with a real first and last name, change role, reset password, rename, remove); every role's name is shown wherever they're referred to, instead of a fixed word like "Jeff", so nothing needs updating if that person changes; everyone signs in with email and password and can change their own; import the bookkeeper's spreadsheet as checklist items; per-item status and a short note; assign items to a specific team member by name; each one has their own filtered list of only what's assigned to them, and updates their own statuses; more than one team member can be added, each with their own separate list; bookkeeper has a live shared view; user login for all roles.
